...
def print_trainables(model):
    ...
    print(f"Trainable: {num_train:,} / {num_total:,} ({100.0*num_train/num_total:.2f}%)")

def encoder_layer_indices_dp(model):
    """Return sorted unique encoder layer indices discovered from param names (works under DataParallel)."""
    names = [n for n,_ in model.module.ast.named_parameters()]
    idxs = set()
    for n in names:
...
def set_all_backbone_frozen_dp(model, frozen: bool = True):
    for p in model.module.ast.parameters():
        p.requires_grad = (not frozen)

def unfreeze_last_k_layers_dp(model, k: int, also_pooler: bool = True):
    """Unfreeze last k encoder layers (and pooler) under DataParallel."""
    # freeze everything first
    set_all_backbone_frozen_dp(model, frozen=True)
    # detect available layers
    idxs = encoder_layer_indices_dp(model)
    if not idxs:
        print("[WARN] Could not detect encoder.layer indices; leaving backbone frozen.")
        return []
    last_idxs = set(idxs[-k:]) if k > 0 else set()

    unfrozen = []
    for n, p in model.module.ast.named_parameters():
        # layers
        m = re.search(r'encoder\.layer\.(\d+)\.', n)
        if m and int(m.group(1)) in last_idxs:
            p.requires_grad = True
            unfrozen.append(n)
        # pooler (optional)
        elif also_pooler and ("pooler" in n or "pooler" in n.lower()):
            p.requires_grad = True
            unfrozen.append(n)
    print(f"Unfroze last {len(last_idxs)} encoder layer(s): {sorted(list(last_idxs))}"
          f"{' + pooler' if also_pooler else ''}")
    return unfrozen

def build_optimizer_head_backbone_dp(model, lr_head=1e-4, lr_backbone=3e-5, wd=1e-5):
    head_params = list(model.module.fc.parameters())
    bb_params   = [p for p in model.module.ast.parameters() if p.requires_grad]
    # If backbone stays frozen, bb_params could be empty; handle both cases:
    if len(bb_params) == 0:
        return AdamW([{'params': head_params, 'lr': lr_head, 'weight_decay': wd}])
    else:
        return AdamW([
            {'params': head_params, 'lr': lr_head,   'weight_decay': wd},
            {'params': bb_params,   'lr': lr_backbone, 'weight_decay': wd},
        ])
...
if __name__ == "__main__":
    ...
    if config['model']['pretrained_path']:
        ...
        state = torch.load(config['model']['pretrained_path'], map_location="cpu")

        # accept either raw state_dict or a dict with 'model_state_dict'
        sd = state.get('model_state_dict', state)

        # add 'module.' only if missing (for DataParallel)
        if any(k.startswith('module.') for k in sd.keys()):
            load_sd = sd
        else:
            load_sd = {f"module.{k}": v for k, v in sd.items()}
    ...
    # Freeze all backbone; train head only (warm-up)
    ...
    # Optimizer: head only for now
    opt = build_optimizer_head_backbone_dp(
        model,
        lr_head=config['training']['lr'],     # your current head LR
        lr_backbone=3e-5,                     # used later when we unfreeze
        wd=1e-5
    )
    ...
    num_total  = sum(p.numel() for p in model.parameters())
    num_train  = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Trainable: {num_train:,} / {num_total:,} "
          f"({100.0*num_train/num_total:.2f}%) — should mainly be the head")
    assert num_train > 0, "No trainable parameters! Check freezing logic."
    
    print("Discovered encoder layers:", encoder_layer_indices_dp(model))
    ...
    # Train epochs ----------------------------------------------------------------------------------
    warmup_epochs = 2          # head-only epochs
    last_k = 2                 # unfreeze the last K encoder blocks
    did_unfreeze = False
    ...
    for epoch in tqdm(range(1, config['training']['max_epochs'] + 1)):
        ...
        # ---- Unfreeze trigger ----
        if (epoch + 1) == warmup_epochs and not did_unfreeze:
            # Unfreeze last K layers (+ pooler), then rebuild optimizer with 2 param groups
            unfreeze_last_k_layers_dp(model, k=last_k, also_pooler=True)
            print_trainables(model)
            opt = build_optimizer_head_backbone_dp(
                model,
                lr_head=config['training']['lr'],  # keep head LR
                lr_backbone=3e-5,                  # small LR for backbone
                wd=1e-5
            )
            # Recreate scheduler tied to the new optimizer (simple + safe)
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                opt, mode='min', factor=0.5,
                patience=config['training']['tr_lr_patience'],
                threshold=1e-4, min_lr=1e-6
